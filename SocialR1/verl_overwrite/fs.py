...
def copy_to_shm(src: str) -> str:
    ...
    """
    # Check if src is a Hugging Face model ID (contains '/' but doesn't start with '/' or 'hdfs://')
    if src and '/' in src and not src.startswith('/') and not src.startswith('hdfs://') and not os.path.exists(src):
        # Return Hugging Face model IDs as is
        print(f"Detected Hugging Face model ID in copy_to_shm: {src}")
        return src
        
    if not os.path.exists("/dev/shm") or not os.path.exists(src):
        return src
...
def copy_to_local(
    ...
    if src and '/' in src and not src.startswith('/') and not src.startswith('hdfs://') and not os.path.exists(src):
        if verbose:
            print(f"Detected Hugging Face model ID: {src}")
        # Return the model ID as is - it will be loaded directly by the model loading code
        return src
        
    local_path = copy_local_path_from_hdfs(
        src, cache_dir=cache_dir, filelock=filelock, verbose=verbose, always_recopy=always_recopy
    )
    if use_shm and os.path.exists(local_path):
...
def copy_local_path_from_hdfs(
    ...
    # Check if src is not empty before checking the last character
    if src and src[-1] == "/":
        raise ValueError(f"Make sure the last char in src is not / because it will cause error. Got {src}")
