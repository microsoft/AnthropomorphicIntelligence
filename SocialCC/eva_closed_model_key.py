...
def _resolve_model_for_chat(model_name: str) -> str:
    
    mn = model_name.strip().lower()
    if mn.startswith("gpt-5"):
        print("[warn] Selected model appears to be GPT-5; some GPT-5 variants are Responses-only. "
              "Falling back to 'gpt-4o' for Chat Completions.", file=sys.stderr)
        return "gpt-4o"
    return model_name


config_file_path = f"./config_openai_key.json"
with open(config_file_path, "r") as f:
    config = json.load(f)
openai_key = config["OPENAI_API_KEY"]
openai.api_key = openai_key

def call_llm(awareness_prompt: str) -> str:
    try:
